module zk_escrow::zk_safe {
    ...
    public fun claim<T: key + store>(
        ...
        let (asset, request) = kiosk::purchase<T>(kiosk, asset_id, coin::zero(ctx));
        (asset, request)
    }
    ...
    public fun prove<T: key + store>(
        ...
        policy_cap: &mut TransferPolicyCap<T>,
        ctx: &mut TxContext
    ) {
        {
    ...
    public fun just_do<T: key + store>(
        publisher: &Publisher,
        asset: T,
        ctx: &mut TxContext
    ) {
